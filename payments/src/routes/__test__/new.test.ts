...
it('returns a 404 when purchasing an order that does not exist', async () => {
  await request(app)
    .post('/api/payments')
    .set('Cookie', signup())
    .send({ token: 'asdsadas', orderId: new mongoose.Types.ObjectId().toHexString() })
    .expect(404);
});

it('retrusn a 401 when purchasing an order that does not belong to the user', async () => {
  ...
  await request(app)
    .post('/api/payments')
    .set('Cookie', signup())
    .send({ token: 'asdsadas', orderId: order.id })
    .expect(401);
});
